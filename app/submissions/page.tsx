...
import { Upload, Link, DollarSign, Calendar, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export default function SubmissionsPage() {
  ...
  const uploadScreenshot = async (file: File): Promise<string> => {
    const storageRef = ref(storage, `submissions/${auth.currentUser?.uid}/${Date.now()}_${file.name}`);
    ...
    <div className="min-h-screen bg-bg">
      ...
      <main className="container mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted">Upload proof of your posts to get cashback approved</p>
        </div>
            ...
            {submissions.length > 0 ? (
              <div className="space-y-4">
                {submissions.map((submission, index) => (
                  <div key={submission.id || index} className="border border-border rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm font-medium text-fg mb-1">Story Link:</p>
                        <a 
                      ...
                      {submission.amount > 0 && (
                        <div>
                          <p className="text-sm font-medium text-fg mb-1">Amount:</p>
                          <p className="text-sm text-muted">₹{submission.amount.toLocaleString()}</p>
                        </div>
                      )}
                    </div>
                    ...
                    {submission.screenshot && (
                      <div className="mt-3">
                        <p className="text-sm font-medium text-fg mb-2">Screenshot:</p>
                        <img 
                    ...
                    {submission.notes && (
                      <div className="mt-3 p-3 bg-accent-light rounded-lg">
                        <p className="text-sm font-medium text-fg mb-1">Admin Notes:</p>
